...
@property {int} fromStateType = 0 {"label" : "From","widget" : "combobox", "values" : [{"label" : "Entry", "value" : -1}, {"label" : "Any", "value" : 1}, {"label" : "Specific State", "value" : 0}]}
...
} = require("Modules/AnimationHelpers.js");
const {
   StateMachine,
   State,
   StateTransition,
} = require("Modules/StateMachine.js");
...
function init(animPlayer) {
   ...
   if (script.parameters) {
      script.parameters.forEach((p) => {
         parameters[p.paramName] = new AnimationParameter(
            p.type,
            p["value" + p.type]
         );
      });
...
/**
 * add state transition that checks on update if certain condition is met
...
function addTransitionBetweenStates(fromStateName, toStateName, config) {
   ...
   if (stateInfoDict[fromStateName] == undefined) {
      print(
         `WARNING, Can not add transition [ ${fromStateName} ]-->[ ${toStateName} ], state [ ${fromStateName} ] is not defined `
      );
      return null;
   }
   if (stateInfoDict[toStateName] == undefined) {
      print(
         `WARNING, Can not add transition [ ${fromStateName} ]-->[ ${toStateName} ], state [ ${toStateName} ] is not defined `
      );
      return null;
   }
   // a combined function that has to return true in order to start transition

   let checkFunctions = getConditionFunctions(config);

   if (config.hasExitTime) {
      if (stateInfoDict[fromStateName].source == StateSource.SingleClip) {
   ...
   // }
   let transition = stateMachine.states[fromStateName].addUpdateTransition(
      toStateName,
      combineFunctions(checkFunctions),
...
function moveToEnd(s) {
   if (s.source == StateSource.SingleClip) {
      putClipAtEndIfNeeded(player, s.clipName);
   } else if (s.source == StateSource.BlendTree) {
...
function getConditionFunctions(t) {
   ...
   t.conditions.forEach((c) => {
      if (!parameters[c.paramName]) {
         ...
         print(
            `ERROR, please define parameter [${c.paramName}] in Parameters section of script UI`
         );
         return;
      }
      ...
      case StateType.Specific:
         if (config.source == StateSource.SingleClip) {
            ...
            if (!player.getClip(config.clipName)) {
               print(
                  `WARNING, Clip ${config.clipName} not found on Animation Player, please check the clip name`
               );
               return;
            }
            ...
            if (config.blendTreeClips.length < 2) {
               print("Error, BlendTree should have at least two clips");
               return;
            }
            let blendTree = createBlendTree(
               config.blendTreeClips,
               config.normalizeClipDuration
            );
            let getParameter = () => {
...
function createBlendTree(clipThresholdPairs, normalize) {
   ...
   for (var i = 0; i < clipThresholdPairs.length; i++) {
      if (
         clipThresholdPairs[i] &&
         player.getClip(clipThresholdPairs[i].clipName)
      ) {
         blendTree.addChild(
            clipThresholdPairs[i].clipName,
            clipThresholdPairs[i].threshold
         );
      }
   }
   return blendTree;
...
function checkTimeToExit(clipName, normalizedTime, delta) {
   let clip = player.getClip(clipName);
   return (
      (player.getClipCurrentTime(clipName) - clip.begin + delta) /
         player.getClip(clipName).duration >=
      normalizedTime
   );
}
...
function showDebug() {
   ...
   if (script.displayStates) {
      ...
      if (stateMachine.isTransitioning) {
         s +=
            stateMachine.currentTransition.currentStateName +
            " -> " +
            stateMachine.currentTransition.nextStateName;
      } else {
...
function isParameterDefined(name) {
   if (parameters[name] == undefined) {
      print(
         `WARNING, Parameter [${name}] is not defined, add parameter in script UI`
      );
      return false;
   }
   return true;
 ...
 */
script.addParameter = function (name, type, defaultValue) {
   parameters[name] = new AnimationParameter(type, defaultValue);
};
/**
 *
 * @param {string} name
